...
def generate_character_script(params: CharacterParams) -> str:
    ...
    script_lines = [
        ...
        "",
        "    try:",
        f'        bpy.context.scene.mblab_character_gender = "{params.gender}"',
        "        if obj.data.shape_keys:",
        "            keys = obj.data.shape_keys.key_blocks",
        '            if "Expressions_mass" in keys:',
        f'                keys["Expressions_mass"].value = {params.mass}',
        '            if "Expressions_height" in keys:',
        f'                keys["Expressions_height"].value = {(params.height - 1.7) / 0.3}',
